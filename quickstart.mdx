...
Clients are who you invoice. Create them manually or import from Google Contacts.
...
<AccordionGroup>
  <Accordion icon="user-plus" title="Create a client">
    ...
    <Tip>Enable **Client Enrichment** to let AI automatically find company details, logo, and address!</Tip>
  </Accordion>

  <Accordion icon="google" title="Import from Google Contacts (optional)">
    Already have contacts in Google? Import them in seconds:

    **Settings** → **Integrations** → Connect Google → **Clients** → **Import from Google Contacts**
  </Accordion>
</AccordionGroup>

<Tip>
  **Ask Every Agent:** "Create a new client named ACME Corp with email contact@acme.com" or "Import my Google Contacts"
</Tip>
