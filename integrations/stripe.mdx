...
Go to [**Settings**](https://app.every.ai/settings) → **Payments** → **Connect Stripe Account** → Choose to create new or connect existing Stripe account → Complete verification → Return to Every.
...
Access via Settings → Finance → View Stripe Dashboard (or directly at stripe.com)
...
**Connection lost:** Settings → Finance → Reconnect Stripe → Log in and reauthorize
